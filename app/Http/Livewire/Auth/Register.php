...
//        $response = Http::withOptions(["verify" => false])->get('https://stat.gov.kz/api/juridical/counter/api/?bin=' . $data . '&lang=ru');
...
//        $response = Http::get('https://stat.gov.kz/api/juridical/counter/api/?bin='.$this->form['bin'].'&lang=ru');
        ...
        $response = Http::withOptions(["verify" => false])->get('https://stat.gov.kz/api/juridical/counter/api/?bin=' . $data . '&lang=ru');
        if ($response->successful()) {
            if (is_null(json_decode($response->body(), true)['obj'])) {
                $this->company_name = '';

            } else {
                $data = json_decode($response->body(), true)['obj']['name'];
                $this->company_name = $data;
            }
//            $this->form['company_name'] = $data;
        }
